    ...
    static class AcpEntitlementWiring
    {
        ...
        internal static void Apply()
        {
            ...
            ConnectionCensus.SetPolicy(new ConnectionPolicy(
                MaxDirect: EnsureLocalDirectMcpAccess(limits.AllowedMcpConnections),
                MaxGateway: limits.AllowedGatewayConnections));
        }

        static int EnsureLocalDirectMcpAccess(int allowedMcpConnections)
        {
            return allowedMcpConnections == 0 ? 1 : allowedMcpConnections;
        }
